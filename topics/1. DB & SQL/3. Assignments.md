...
1. **DDL Script:** Write an init_schema.sql file to create all tables.  
   * Use DECIMAL for currency.  
   * Constraints: Price > 0, Quantity >= 0.  
2. **Soft Delete:** Apply soft delete for the Products table (is_deleted column).  
3. **Data Seeding:** Write a seed_data.sql file to create sample data:  
   * Users: 50 people.  
...
**Requirement:** Write a report_queries.sql file to solve requests from the Board of Directors:
...
1. **Order Transaction:** Write Stored Procedure sp_CreateOrder:  
